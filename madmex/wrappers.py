...
def segment(tile, algorithm, segmentation_meta, band_list, extra_args):
...
def write_predict_result_to_raster(id, predict_name, resolution, path_destiny):
    ...
    Args:
        ...
        path_destiny (str): Path that will hold results. Only writes to file system are supported now.
    ...
    with fiona.open(path_seg) as src:
        ...
        meta = {'driver': 'GTiff',
                ...
                'compress': 'lzw',
                'nodata': 0}
        filename = os.path.join(path_destiny, '%s.tif' % segmentation_name_classified)
