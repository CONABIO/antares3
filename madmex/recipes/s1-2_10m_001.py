...
def run(tile, center_dt, path):
    """Basic datapreparation  with sentinel 1 and 2 combined
    ...
    try:
        ...
        nc_filename = os.path.join(path, 's1-2_10m_001_%d_%d_%s.nc' % (tile[0][0], tile[0][1], center_dt))
        # Load Landsat sr
        if os.path.isfile(nc_filename):
            logger.warning('%s already exists. Returning filename for database indexing', nc_filename)
            return nc_filename
        sr_0 = xr.merge([GridWorkflow.load(x, dask_chunks={'x': 5000, 'y': 5000}) for x in tile[1]])
        ...
        sr_0=sr_1=sr_mean=ndvi_max=ndvi_min=ndmi_max=ndmi_min=terrain=combined=sar=sar_mean=None
        gc.collect()
        return nc_filename
