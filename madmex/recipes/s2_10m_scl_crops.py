...
def run(tile, center_dt, path):
    ...
    try:
        ...
        if os.path.isfile(nc_filename):
            ...
            return nc_filename
        sr_0 = xr.merge([GridWorkflow.load(x, dask_chunks={'x': 700, 'y': 700}) for x in tile[1]])
        ...
        sr_1 = sr_1.chunk({'time': -1, 'x': 700, 'y': 700})
        ...
        indexes = sr_1['ndvi'].where(np.isfinite(sr_1['ndvi']), other=-99).data.argmax(axis=0)
        begin_date = str(sr_1['ndvi']['time'][0].values).split('-')[0] + '-01-01'
        begin = datetime.strptime(begin_date, '%Y-%m-%d').timestamp()*1e-4 - 100
        ndvi_argmax = xr.DataArray(sr_1['ndvi']['time'].values[indexes].astype(datetime)*1e-13 - begin,
