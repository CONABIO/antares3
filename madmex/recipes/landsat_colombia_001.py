...
def run(tile, center_dt, path):
    ...
    try:
        ...
        nc_filename = os.path.join(path, 'landsat_colombia_002_%d_%d_%s.nc' % (tile[0][0], tile[0][1], center_dt))
        # Load Landsat sr
        ...
        sr_0 = xr.merge([GridWorkflow.load(x, dask_chunks={'x': 1860, 'y': 1860})
                         for x in tile[1]])
