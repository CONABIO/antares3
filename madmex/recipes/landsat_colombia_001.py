...
def run(tile, center_dt, path):
    ...
    try:
        ...
        nc_filename = os.path.join(path, 'landsat_colombia_001_%d_%d_%s.nc' % (tile[0][0], tile[0][1], center_dt))
        # Load Landsat sr
        ...
        sr_0 = xr.merge([GridWorkflow.load(x, dask_chunks={'latitude': 1860, 'longitude': 1860, 'time': 3})
                         for x in tile[1]])
        ...
        terrain = dc.load(product='srtm_cgiar_colombia', like=sr_0,
                         ...
                         dask_chunks={'latitude': 1860, 'longitude': 1860, 'time': 3})
        dc.close()
        # Merge dataarrays
