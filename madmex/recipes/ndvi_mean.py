...
def run(tile, gwf, center_dt):
    ...
    Computes median NDVI for a landsat collection over a given time frame
    ...
    try:
        ...
        nc_filename = os.path.expanduser('~/datacube_ingest/recipes/ndvi_median/ndvi_median_%d_%d_%s.nc' % (tile[0][0], tile[0][1], center_dt))
        # TODO: Is it a good idea to check if file already exist and skip processing if it does?
        # Load Landsat sr
        sr = gwf.load(tile[1], dask_chunks={'x': 1000, 'y': 1000})
        # Compute ndvi
        sr['ndvi'] = (sr.nir - sr.red) / (sr.nir + sr.red) * 10000
        ndvi = sr.drop(['pixel_qa', 'blue', 'red', 'green', 'nir', 'swir1', 'swir2'])
        # Run temporal reductions and rename DataArrays
        ndvi_median = ndvi.mean('time', keep_attrs=True)
        ndvi_median = ndvi_median.astype('int16')
        ndvi_median.attrs['crs'] = sr.attrs['crs']
        write_dataset_to_netcdf(ndvi_median, nc_filename)
        return nc_filename
