...
def run(tile, center_dt, path):
    ...
    try:
        ...
        nc_filename = os.path.join(path, 'landsat_guyana_001_%d_%d_%s.nc' % (tile[0][0], tile[0][1], center_dt))
        # Load Landsat sr
        ...
        dc = datacube.Datacube(app = 'landsat_guyana_%s' % randomword(5))
        terrain = dc.load(product='srtm_cgiar_guyana', like=sr_0,
